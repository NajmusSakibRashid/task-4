...
export async function PUT(request, { params }) {
    try {
        ...
        for (let record of recordset) {
            // console.log(record)
            if (selected[record.id] || selected[-1]) {
                await pool.request().query(`update users set status=${status} where id=${record.id}`);
            }
        }
        const { recordset: db_user } = await pool.request().query(`select * from users where id=${params.userId}`);
        // console.log(db_user);
